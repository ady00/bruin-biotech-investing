...
function AnalystCard({ member }: { member: TeamMember }) {
  // Use top positioning for Andrew Spencer's photo
  const imagePosition = member.name === 'Andrew Spencer' ? 'object-top' : 'object-center';
...
export function AnalystsSection({ analysts }: { analysts: TeamMember[] }) {
        ...
        >
          <motion.div variants={itemVariants} className="text-center mb-12">
            ...
            <p className="text-[var(--muted)] max-w-2xl mx-auto">
              Our analysts drive research and analysis across the biotech landscape.
            </p>
          </motion.div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-8">
            {analysts.map((analyst) => (
              <AnalystCard key={analyst.id} member={analyst} />
            ))}
          </div>
        </motion.div>
      </div>
